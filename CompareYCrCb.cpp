...
CompareYCrCb::CompareYCrCb(){
         
}

void CompareYCrCb::viewGeneralMap(map<string, map<string, list<string> > > generalMap){
...
map < string , double > CompareYCrCb::distanciasGeneralesBGR(map<string, map<string, list<string> > > generalMap){
...
double  CompareYCrCb::media(map< string , list<string> > mapa){
    ...
    try{
            ...
            for(size_t i=0;i<test.size();i++){
                ...
                Mat imgTest = imread(*it, IMREAD_COLOR);//carga img
                ...
                for(size_t j=0;j<train.size();j++){
                    ...
                    Mat imgTrain = imread(*jt, IMREAD_COLOR);//carga img
                    cvtColor(imgTrain, imgTrain,COLOR_BGR2YCrCb); 
...
double CompareYCrCb::calcularDistancia(vector<Mat> canales_Test,vector<Mat> canales_Train){
...
double CompareYCrCb::calcDistForCanal(Mat canales_Test,Mat canales_Train ){
    double sum=0;    
...
map < string , bool > CompareYCrCb::aciertosFallosBGR(map<string, map<string, list<string> > > nuevoMapa, map< string , double > distanciasGeneralesBGR){    
...
bool CompareYCrCb::compararAllTrain(vector<Mat> canalesBGR_Test,map< string, list<string> > train,map<string,double> distanciasGeneralesBGR,string claseTest){
    float distancia=0.0;
    bool fin;
    float rango[] = {0, 256};
    const float *rangoHistograma = {rango};       
...
bool CompareYCrCb::verificarClase(double distancia,string claseTest,map<string,double> distanciasGeneralesBGR){
    double aux=99999.0;
    string clase;
...
vector<Mat> CompareYCrCb::calcularHistBGR(string img){
    //cout<<img<<endl;
    vector<Mat> canalesBGR; 
    try{

        Mat imagen = imread(img, IMREAD_COLOR);//carga img
        cvtColor(imagen, imagen,COLOR_BGR2YCrCb);
