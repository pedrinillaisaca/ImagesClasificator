...
CompareHSV::CompareHSV(){
         
}

void CompareHSV::viewGeneralMap(map<string, map<string, list<string> > > generalMap){
...
map < string , double > CompareHSV::distanciasGeneralesBGR(map<string, map<string, list<string> > > generalMap){
...
double  CompareHSV::media(map< string , list<string> > mapa){
    ...
    try{
            ...
            for(size_t i=0;i<test.size();i++){
                ...
                for(size_t j=0;j<train.size();j++){
                    ...
                    cvtColor(imgTrain, imgTrain,COLOR_RGB2HSV); 
...
double CompareHSV::calcularDistancia(vector<Mat> canales_Test,vector<Mat> canales_Train){
...
double CompareHSV::calcDistForCanal(Mat canales_Test,Mat canales_Train ){
    double sum=0;    
...
map < string , bool > CompareHSV::aciertosFallosBGR(map<string, map<string, list<string> > > nuevoMapa, map< string , double > distanciasGeneralesBGR){    
...
bool CompareHSV::compararAllTrain(vector<Mat> canalesBGR_Test,map< string, list<string> > train,map<string,double> distanciasGeneralesBGR,string claseTest){
    float distancia=0.0;
    bool fin;
    float rango[] = {0, 256};
    const float *rangoHistograma = {rango};       
...
bool CompareHSV::verificarClase(double distancia,string claseTest,map<string,double> distanciasGeneralesBGR){
    double aux=99999.0;
    string clase;
...
vector<Mat> CompareHSV::calcularHistBGR(string img){
    //cout<<img<<endl;
    vector<Mat> canalesBGR; 
    try{

        Mat imagen = imread(img, IMREAD_COLOR);//carga img
        cvtColor(imagen, imagen,COLOR_RGB2HSV);
